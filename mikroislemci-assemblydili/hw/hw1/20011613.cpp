...
int main() {
    ...
    __asm {
        ...
        inverse_loop:
            MOV AL, [ESI]
            MOV BL, 255
            SUB BL, AL              // negatif filtre uygulama için negatif deðer hesaplanýr
            MOV [EDI], BL

            INC ESI                 // sonraki pixele geç
            INC EDI
            LOOP inverse_loop
    }
    ...
    __asm {
        ...
        find_min_cdf :
            ...
            TEST EAX, EAX                          // cdf[edi] == 0 kontrolü
            JNZ found_min_cdf                      // tüm pixelleri gez
            INC EDI                                
            CMP EDI, 256
            JL find_min_cdf
            JMP normalize_cdf
